...
def receive_file(sock_client, dechiff, decrypt_hmac, hmac_received, window):
    ...
    with open('received_file_B.txt', 'wb') as f:
...
def server(window):
    ...
    sock_server.bind(('localhost', 1002))
    sock_server.listen(20)
    window['-OUTPUT-'].print("User B listening on port 1000...")
...
def client(window, file_path):
    ...
    sock_client.connect(('localhost', 1000))
    window['-OUTPUT-'].print("Client A connected : ")
...
def main():
    ...
    window = sg.Window("User B Interface", layout)
